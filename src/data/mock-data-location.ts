import type {StoryLocation} from '../services/AirtableService';

export const initialStoryLocations: StoryLocation[] = [
  {
    "id": 1,
    "title": "Punto 1: Punto Cero: Endémico / Entrada Campito",
    "description": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae; Sed vitae justo eget magna fermentum iaculis. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Mauris quis nisl non tellus tempor feugiat. Donec ut elementum nisl. Suspendisse potenti. Nam non sem at nisl tempor convallis.",
    "description_en": "",
    "latitude": 4.60155,
    "longitude": -74.06648,
    "audioFile": "audio1.mp3",
    "played": false
  },
  {
    "id": 2,
    "title": "Punto 2: Patio Central del Campito (Claustro)",
    "description": "Proin id urna egestas, sodales tellus a, interdum metus. Mauris rhoncus vulputate neque, sed pretium eros. Etiam quis nisl at justo pretium. Curabitur pretium tincidunt lacus. Nulla gravida orci a odio. Nullam varius, turpis et commodo pharetra, est eros bibendum elit, nec luctus magna felis sollicitudin mauris. Integer egestas, ante et vulputate volutpat, eros pede semper est, vitae luctus metus libero eu augue.",
    "description_en": "",
    "latitude": 4.60140,
    "longitude": -74.06630,
    "audioFile": "audio2.mp3",
    "played": false
  },
  {
    "id": 3,
    "title": "Punto 3: Eje Ambiental (Puente Peatonal)",
    "description": "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt. Neque porro quisquam est, qui dolorem ipsum quia.",
    "description_en": "",
    "latitude": 4.60110,
    "longitude": -74.06590,
    "audioFile": "audio3.mp3",
    "played": false
  },
  {
    "id": 4,
    "title": "Punto 4: Parque Espinosa (Monumento)",
    "description": "Quis autem vel eum iure reprehenderit qui in ea voluptate velit esse quam nihil molestiae consequatur, vel illum qui dolorem eum fugiat quo voluptas nulla pariatur? At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque corrupti quos dolores et quas molestias excepturi sint occaecati cupiditate non provident, similique sunt in culpa qui officia deserunt mollitia.",
    "description_en": "",
    "latitude": 4.60280,
    "longitude": -74.06710,
    "audioFile": "audio4.mp3",
    "played": false
  },
  {
    "id": 5,
    "title": "Punto 5: Iglesia Nuestra Señora de las Aguas",
    "description": "Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus, omnis voluptas assumenda est, omnis dolor repellendus. Temporibus autem quibusdam et aut officiis debitis aut rerum necessitatibus saepe eveniet ut et voluptates repudiandae sint et molestiae non recusandae. Itaque earum rerum hic tenetur.",
    "description_en": "",
    "latitude": 4.60190,
    "longitude": -74.06550,
    "audioFile": "audio5.mp3",
    "played": false
  },
  {
    "id": 6,
    "title": "Punto 6: Edificio Richard (Antiguos Servicios)",
    "description": "Vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque corrupti quos dolores et quas molestias excepturi sint occaecati cupiditate non provident, similique sunt in culpa qui officia deserunt mollitia animi, id est laborum et dolorum fuga. Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque.",
    "description_en": "",
    "latitude": 4.60090,
    "longitude": -74.06610,
    "audioFile": "audio6.mp3",
    "played": false
  },
  {
    "id": 7,
    "title": "Punto 7: Plazoleta de la Pola (Referencia)",
    "description": "Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt. Neque porro quisquam est, qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit, sed quia non numquam eius modi tempora incidunt ut labore et dolore magnam aliquam quaerat voluptatem. Ut enim ad minima veniam, quis nostrum exercitationem.",
    "description_en": "",
    "latitude": 4.60050,
    "longitude": -74.06580,
    "audioFile": "audio7.mp3",
    "played": false
  },
]